...
console.log('✅ ' + (process.env.DB_TYPE || 'postgresql').toUpperCase() + ' Order Storage Enabled');
...
app.get('/orders/:orderId/status', asyncHandler(async (req, res) => {
  const { orderId } = req.params;
...
app.post('/payment/create', validateOrder, asyncHandler(async (req, res) => {
  ...
  const transactionDetails = {
    ...
    enabled_payments: (paymentMethod === 'bca' ? ['bca_va'] : paymentMethod === 'bni' ? ['bni_va'] : paymentMethod === 'bri' ? ['bri_va'] : paymentMethod === 'mandiri' ? ['echannel'] : paymentMethod === 'qris' ? ['qris'] : paymentMethod === 'gopay' ? ['gopay'] : paymentMethod === 'shopeepay' ? ['shopeepay'] : ['credit_card', 'bca_va', 'bni_va', 'bri_va', 'echannel', 'gopay', 'shopeepay', 'qris']),
  ...
  try {
    ...
    logger.error({ error: error.message, orderId }, 'Failed to queue notifications');
...
app.post('/webhooks/midtrans', verifyMidtransSignature, asyncHandler(async (req, res) => {
  logger.info({ webhook: req.body }, 'Midtrans webhook received');

  const { order_id, transaction_status, transaction_id } = req.body;

  // 1. Idempotency Check (Using payment_events)
  const existingEvent = await database.get(
    'SELECT processed FROM payment_events WHERE transaction_id = $1',
    [transaction_id]
  );

  if (existingEvent && existingEvent.processed) {
    logger.warn({ transaction_id, order_id }, 'Webhook already processed, skipping');
    return res.status(200).json({ success: true, message: 'Already processed' });
  }

  // 2. Record Event (Audit Log)
  if (!existingEvent) {
    await database.query(
      'INSERT INTO payment_events (order_id, transaction_id, signature_valid, raw_payload) VALUES ($1, $2, $3, $4)',
      [order_id, transaction_id, true, JSON.stringify(req.body)]
    );
  }
  ...
  if (transaction_status === 'settlement' || transaction_status === 'capture') {
    ...
    if (order) {
      ...
      try {
        ...
        logger.info({ orderId: order_id }, 'Success notifications queued to outbox');
      } catch (error) {
        logger.error({ error: error.message, orderId: order_id }, 'Failed to queue success notifications');
      }
    }
  }
  ...
  await database.query(
    'UPDATE payment_events SET processed = TRUE, processed_at = NOW() WHERE transaction_id = $1',
    [transaction_id]
  );
